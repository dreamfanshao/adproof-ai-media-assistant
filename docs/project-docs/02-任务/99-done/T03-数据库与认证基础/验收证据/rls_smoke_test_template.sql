-- RLS smoke-test template. DO NOT add real production user IDs to Git.
-- Prerequisite: register two disposable Supabase Auth users, then replace the
-- two placeholders below in a temporary local copy of this file.
-- Run only against a disposable local/test Supabase project.

begin;

-- Seed as postgres/service role after replacing placeholders.
insert into public.projects (id, user_id, idempotency_key, name, product_name)
values
  ('10000000-0000-0000-0000-000000000001', '__USER_A_UUID__', '10000000-0000-0000-0000-000000000011', 'A 项目', 'A 产品'),
  ('20000000-0000-0000-0000-000000000001', '__USER_B_UUID__', '20000000-0000-0000-0000-000000000011', 'B 项目', 'B 产品');

insert into public.knowledge_bases (id, user_id, scope, name)
values
  ('10000000-0000-0000-0000-000000000002', '__USER_A_UUID__', 'private', 'A 私有库'),
  ('20000000-0000-0000-0000-000000000002', '__USER_B_UUID__', 'private', 'B 私有库'),
...
values
  ('10000000-0000-0000-0000-000000000004', '__USER_A_UUID__', '10000000-0000-0000-0000-000000000014', 'A 审核', 'A 正文'),
  ('20000000-0000-0000-0000-000000000004', '__USER_B_UUID__', '20000000-0000-0000-0000-000000000014', 'B 审核', 'B 正文');

insert into private.jobs (
  id, user_id, type, idempotency_scope, idempotency_key
) values (
  '10000000-0000-0000-0000-000000000005', '__USER_A_UUID__',
...
insert into private.upload_intents (
  ...
  '10000000-0000-0000-0000-000000000006', '__USER_A_UUID__', 'audit_asset',
  '__USER_A_UUID__/10000000-0000-0000-0000-000000000006/a.png',
  'a.png', 'image/png', 100, repeat('b', 64), now() + interval '10 minutes'
);

insert into storage.objects (bucket_id, name)
values
  ('audit-assets', '__USER_A_UUID__/seed/a.png'),
  ('audit-assets', '__USER_B_UUID__/seed/b.png');
...
begin
  ...
  if public.can_upload_to_storage('audit-assets', '__USER_B_UUID__/unknown/b.png') then
  ...
  begin
    ...
    values ('__USER_A_UUID__', '20000000-0000-0000-0000-000000000001', '越权分组');
  exception
...
-- Test as user B.
set local role authenticated;
select set_config('request.jwt.claim.sub', '__USER_B_UUID__', true);
select set_config('request.jwt.claim.role', 'authenticated', true);
